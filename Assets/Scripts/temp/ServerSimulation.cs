...
public class ServerSimulation
{
        ...
        CardStruct[] tempCards=new CardStruct[108];
        {
            ...
            foreach (CardDeck deck in Enum.GetValues(typeof(CardDeck)))
            {
                foreach (CardSuit suit in Enum.GetValues(typeof(CardSuit)))
                {
                    ...
                    {
                        if(value==CardValue.Joker&&(suit==CardSuit.Clubs||suit==CardSuit.Hearts)) continue;
    ...
    public int AddPlayer()
    {
        ...
        {
            temp = new Player[players.Length+1];
    ...
    private void SendUncensored(Data data)
    {
        for (int i = 0; i < players.Length; i++)
        {
            SendData(data,i);
    ...
    private void SendCensored(Data data,int id)
    {
        // SendUncensored(data);
        SendData(data,id);
        for (int i = 0; i < players.Length; i++)
    ...
    private Data CensorData(Data data)
    {
        ...
        {
            cards[i] = cards[i].Censored();
        }

        return new Data(data.dataHeader,cards);
    }
        ...
        if (isSetupTime)
        {
            switch (data.dataHeader.gameEvent)
            {
                case GameEvent.SwapCards:
                    return SwapCards(id,data.cards);
        ...
        else if(id==turn)
        {
            switch (data.dataHeader.gameEvent)
            {
                ...
                case GameEvent.PlayCards:
                    return PlayCards(data.cards);
                case GameEvent.MoveShownCard:
                    return MoveShownCard(data.cards[0]);
                case GameEvent.MoveHiddenCard:
                    return MoveHiddenCard(data.cards[0]);
                default:
                    return false;
            }
        }
    ...
    private bool Shanked()
    {
        ...
        Data data = new Data(new DataHeader(turn,GameEvent.Shanked),playPile.ToArray());
        SendCensored(data,turn);
    ...
    private bool PlayCards(CardStruct[] cards)
    {
        ...
        Data data = new Data(new DataHeader(turn,GameEvent.PlayCards),cards);
        SendUncensored(data);
    ...
    private bool MoveShownCard(CardStruct card)
    {
        ...
        Data data = new Data(new DataHeader(turn,GameEvent.MoveShownCard),new []{card});
        SendCensored(data,turn);
    ...
    private bool MoveHiddenCard(CardStruct card)
    {
        ...
        Data data = new Data(new DataHeader(turn,GameEvent.MoveHiddenCard),new []{card});
        SendCensored(data,turn);
    ...
    private void SwapHand(int offset)
    {
        ...
        List<CardStruct>[] handsCopy = new List<CardStruct>[players.Length];
        for(int i=0;i<players.Length;i++)
        {
            handsCopy[i]=players[i].mainHand;
        }


        for(int i=0;i<players.Length;i++)
        {
            players[i].mainHand=handsCopy[i+offset];
            Data data = new Data(new DataHeader((byte)i,GameEvent.SwapHand),handsCopy[i+offset].ToArray());
            SendCensored(data,i);
    ...
    private void DrawCards()
    {
        ...
        Data data = new Data(new DataHeader(turn,GameEvent.DrawCards),cards);
        SendCensored(data,turn);
    ...
    private void StartTurn()
    {
        ...
        SendUncensored(new Data(new DataHeader(turn,GameEvent.StartTurn),Array.Empty<CardStruct>()));

        turnOffset=1;

        static bool HasCards(Player player)
        {
    ...
    {
        Data data = new Data(new DataHeader(turn,GameEvent.ClearPile),playPile.ToArray());        
        SendUncensored(data);
    ...
    {
        Data data = new Data(new DataHeader(turn,GameEvent.ClearPile),cards);        
        SendUncensored(data);
    ...
    private void DealHiddenCards(byte id)
    {
        ...
        Data data = new Data(new DataHeader(id,GameEvent.DealHiddenCards),cards);
        SendCensored(data,id);
    ...
    private void DealShownCards(byte id)
    {
        ...
        CardStruct[] cards = new CardStruct[3];
        for (int i = 0; i < 3; i++) cards[i] = drawPile.Pop();
        Data data = new Data(new DataHeader(id,GameEvent.DealShownCards),cards);
        SendUncensored(data);
    ...
    private void DealHandCards(byte id)
    {
        ...
        CardStruct[] cards = new CardStruct[3];
        for (int i = 0; i < 3; i++) cards[i] = drawPile.Pop();
        Data data = new Data(new DataHeader(id,GameEvent.DealHandCards),cards);
        SendCensored(data,id);
    ...
    private bool SwapCards(byte id,CardStruct[] cards)
    {
        ...
        Data data = new Data(new DataHeader(turn,GameEvent.SwapCards),cards);        
        SendUncensored(data);
    ...
    private bool Ready(byte id)
    {
        ...
        Data data = new Data(new DataHeader(id,GameEvent.Ready),Array.Empty<CardStruct>());        
        SendUncensored(data);
